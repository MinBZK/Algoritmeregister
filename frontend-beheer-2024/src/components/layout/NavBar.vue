...
const navOptions: ComputedRef<NavOptions[]> = computed(() => {
  ...
  const options = [
    ...
    },
    {
      name: 'Beheer',
      to: '/beheer',
      highlight: route.name?.toString().includes('beheer') || false,
    },
    {
      name: 'Archief',
      to: '/archief',
      highlight: route.name == 'archive.index',
    },
    {
      name: 'Dashboard',
      to: '/dashboard',
  ...
  return options.filter((option) => {
    if (!authStore.canAccesBeheer && option.to === '/beheer') return false
    ...
    if (!authStore.canAccessArchive && option.to === '/archief') return false
    return true
  })
})
</script>
