...
import { countWithoutHTMLTags, initSpellchecker, toRichTextFormat } from '@/utils/editor'
...
// onMounted(() => {
//   if (editor.value) {
//     initSpellchecker(editor.value.$el)
//   }
// })
const editorStyle = { fontSize: '.95rem' }
const formats = computed(() =>
  props.allowedHtmlTags
    ? toRichTextFormat(props.allowedHtmlTags)
    : []
)
...
const bindings = computed(() => {
  ...
  const hasListFormat = formats.value.filter((format) => format.class === 'list').length !== 0
  if (hasListFormat) {
    return { ...allBindings }
  }
  Object.keys(allBindings).forEach((key) => {
